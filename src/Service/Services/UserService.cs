...
namespace Service.Services
{
    public class UserService : BaseService, IUserService
    {
        ...
        public Guid CreateDriver(DataInput<DriverInput> requestDto) => Create(requestDto.CurrentUser, UserTypeEnum.Driver, ErrorCodeEnum.UserDriverExisted, ErrorCodeEnum.EmailDriverExisted, EnumIDGenerate.Driver, requestDto.Dto);
        ...
        public void Update(UpdateInput updateInput)
            => Update(updateInput, updateInput.DataUpdate is ManagerUpdateInput ? ErrorCodeEnum.EmailManagerExisted : updateInput.DataUpdate is StaffUpdateInput ? ErrorCodeEnum.EmailStaffExisted : ErrorCodeEnum.EmailDriverExisted);
        ...
        public UserOutput View(Guid id)
        {
            ...
            return new UserOutput(row, row.UserType.Equals(UserTypeEnum.Staff) || row.UserType.Equals(UserTypeEnum.Driver) ? GetGroupContact(row.Groups) : null);
        }
        ...
        public CountTotalUsers CountTotalUsers(string currentUser)
        {
            ...
            return allUsers.ToList().Count() > 0 ? new CountTotalUsers
            {
                ...
                Driver = allUsers.Where(x => x.UserType.Equals(UserTypeEnum.Driver.ToString())).Count(),
            } : new CountTotalUsers();
        }
        ...
        {
            var Driver = GetUserContact(currentUser);
            if (Driver.UserType.Equals(UserTypeEnum.Driver))
            {
                if (Driver != null)
                {
                    Driver.Status = Driver.Status.Equals(StatusEnum.Active) ? StatusEnum.EndOfDay : Driver.Status;
                    Driver.LastUpdateDate = Clock.Now;
                }
        ...
        public SearchOutput SearchDriver(DataInput<SearchInput> requestDto)
            => Search(requestDto, UserTypeEnum.Driver, 8);
        ...
        private IEnumerable<DropdownList> GetUsersHasNotBeenAssignedByGroups(UserTypeEnum userType, string countryId, string groups = null)
        {
            ...
            IEnumerable<User> TDriver = GetAllType(UserTypeEnum.Driver, countryId);
            switch (userType)
            {
                ...
                case UserTypeEnum.Staff:
                    ...
                    TDriver = TDriver.Where(e => !TStaff.Any(s => s.Users.SplitTrim(_comma).Any(x => x.Equals(e.Code)))).AsQueryable()
                        .WhereIf(!string.IsNullOrEmpty(groups), x => groups.SplitTrim(_comma).Any(g => g.Equals(x.Groups)));

                    return TDriver.Count() > 0 ?
                        TDriver.AsQueryable().Select(row => new DropdownList(row.Code, row.Username)).AsEnumerable()
                        : new List<DropdownList>();
        ...
        {
            if (!(dataObject is ManagerInput || dataObject is StaffInput || dataObject is DriverInput))
                throw new BadData();
            ...
            else
            {
                ...
                User user = new User
                {
                    ...
                    Users = dataObject is DriverInput ? null : GenerateUsers(userType, (string)dataObject.CountryId, Groups),
                    Groups = Groups,
        ...
        private bool AllowUnselectGroups(string country, string groups, string users, User user)
        {
            ...
            if (!user.UserType.Equals(UserTypeEnum.Driver) && returnResult && userArr.Length > 0)
            {
        ...
        private void Update(UpdateInput requestDto, ErrorCodeEnum exitedEmail)
        {
            ...
            if (!(dataInput is ManagerUpdateInput || dataInput is StaffUpdateInput || dataInput is DriverUpdateInput))
                throw new BadData();
            ...
            if (user != null)
            {
                ...
                if (dataStatus)
                {
                    ...
                    if (dataInput is DriverUpdateInput)
                    {
                ...
                if (!AllowUnselectGroups((string)dataInput.CountryId,
                    ...
                    dataInput is DriverUpdateInput ? null : (string)dataInput.Users,
                    user))
                {
                ...
                user.Users = dataInput is DriverUpdateInput ? null : user.CountryId.Equals((string)dataInput.CountryId) ? (string)dataInput.Users : string.Empty;
        ...
        private SearchOutput Search(DataInput<SearchInput> requestDto, UserTypeEnum searchUserType, int fieldNumber)
        {
            ...
            IQueryable<UserOutput> queryResult = GetAllType(user.UserType.Equals(UserTypeEnum.SuperAdmin) ? searchUserType : user.UserType, user.CountryId ?? null)
                .Select(row => new UserOutput(row, row.UserType.Equals(UserTypeEnum.Staff) || row.UserType.Equals(UserTypeEnum.Driver) ? GetGroupContact(row.Groups) : null));
        ...
        private IQueryable<UserOutput> SearchAuthority(IQueryable<UserOutput> queryResult, User user, UserTypeEnum searchUserType)
        {
            ...
            switch (searchUserType)
            {
                ...
                case UserTypeEnum.Staff:
                    ...
                    IEnumerable<UserOutput> DriverUsers = GetAllType(UserTypeEnum.Driver, user.CountryId ?? null)
                        .Select(row => new UserOutput(row, row.UserType.Equals(UserTypeEnum.Staff) || row.UserType.Equals(UserTypeEnum.Driver) ? GetGroupContact(row.Groups) : null));
                    IEnumerable<UserOutput> filterStaff = queryResult.Where(staff => !string.IsNullOrEmpty(staff.Users)).ToList();
                    if (filterStaff.Count() == 0)
                        return new List<UserOutput>().AsQueryable();
                    IEnumerable<UserOutput> result = DriverUsers.Where(Driver => filterStaff.Any(staff => staff.Users.SplitTrim(_comma).Any(x => x.Equals(Driver.Code))));
                ...
                case UserTypeEnum.Driver:
                    ...
                    IQueryable<UserOutput> DriverUser = GetAllType(UserTypeEnum.Driver, user.CountryId ?? null).Select(row => new UserOutput(row, null));
                    return DriverUser.Where(Driver => user.Users.SplitTrim(_comma).Any(x => x.Equals(Driver.Code)));
        ...
        private IQueryable<UserOutput> GetUserListByUser(User user)
        {
            ...
            switch (user.UserType)
            {
                case UserTypeEnum.Manager:
                    ...
                    IEnumerable<UserOutput> Driver = SearchAuthority(users, user, UserTypeEnum.Driver);
                    var result = Staff.Union(Driver);
                    return result.AsQueryable();

                case UserTypeEnum.Staff:
                    return SearchAuthority(users, user, UserTypeEnum.Driver);
